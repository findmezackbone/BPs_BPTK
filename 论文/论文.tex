...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.6\textwidth}
    \centering
    \resizebox{1\textwidth}{!}{\input{./figs/pcode1.pgf}}
    \caption{$A_{plasma}(t)$的相对误差的十进对数}
  \end{subfigure}
  \begin{subfigure}{0.6\textwidth}
    \centering
    \resizebox{1\textwidth}{!}{\input{./figs/pcode1_1.pgf}}
...
稀疏网格法\cite{15}是一种可用于在高维参数空间取样的高效方法. 以$m$维参数空间下的同维度等距的网格为例, 有如下的稀疏网格生成迭代步骤:
...
\begin{enumerate}
  ...
  \item 
  当$p = 1$时, $\forall 1 \leq i \leq m$ , 将中心点$\bar{x}$在第$i$维度正负两个方向上与之距离为$(u_i - l_i)\times 2^{-(p+1)}$的两个点加入至起点集中$S$, 共有$2*m*2^{m\times (p-1)}$个新的起点被加入至起点集$S$中, 删去起点集中除了新起点外的起点.\\
  ...
  共有$2*m*2^{m\times (p-1)}$个新的起点被加入至起点集$S$中, 删去起点集中除了新起点外的起点. 

  \item $p = p + 1$, 并跳转至步骤2.
  \end{enumerate}

以二维空间$[0,1]\times[0,1]$内中心点为$(0.5,0.5)$的稀疏网格为例, 绘制复杂度$k=2,3,5$时的稀疏网格散点图如图\ref{稀疏网格}所示.
\begin{figure}[H]
  \centering

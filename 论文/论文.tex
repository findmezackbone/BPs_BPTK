...
微分方程组求解, 该函数默认使用LSODA数值格式. 根据Hu等人\cite{11}的实验, 每个解的初值都设置为0, 时间网格设置为$range(0,75,0.005)$, 时间步长为$0.005h$, 共$15000$个时间节点.
...
分别运行150次前人模型求解代码与本文复现的模型求解代码, 取单次运行的平均, 得到本文的模型求解单次平均运行时间为$0.1086s$, 前人的单词平均运行时间为$18.1621s$,

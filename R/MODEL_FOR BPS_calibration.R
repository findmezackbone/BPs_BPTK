...
################### ALL FOR BISPHENOL S ################### 
#################import data ###############
x202111_BPS <- read.csv("D://1st//Project_pharmacy//Documents//Volunteer_data.csv")
para_BP_individual <- read.csv("D://1st/Project_pharmacy//R_language//para_BP_individual.csv")



x202111_BPS$EXP_Time=round(x202111_BPS$EXP_Time,2) #将真实数据的时间戳四舍五入为一位小数
 
t_data_s1=x202111_BPS$EXP_Time 
c_data_s1=x202111_BPS$SUM_t_BPS/x202111_BPS$dose_bps_avail*1000*1000 #总BPS:BPS+BPSg
f_data_s1=x202111_BPS$A_F_BPS/x202111_BPS$dose_bps_avail*1000*1000 #未结合的BPS

urine_init_per=matrix(NA,55,8)   #提前分配内存
urine_out_per=matrix(NA,15001,8)
#################import data ###############

theta_s1=c(14.91,2.78,4.707) #给待优化参数一个初值

##############################_____CALIBRATION__________________##########################
cost_fun_bps1 <- function(theta, t_data, c_data,f_data) {
  # Unpack parameter vector
  for (i in 1:4) {
   
#################### pars ##############################
gender=as.numeric(para_BP_individual[2,i+2])
bw= as.numeric(para_BP_individual[1,i+2])
QCC = as.numeric(para_BP_individual[3,i+2])
QliverC = as.numeric(para_BP_individual[4,i+2])
QskinC =as.numeric( para_BP_individual[7,i+2])
QgonadC=as.numeric(para_BP_individual[8,i+2])
QfatC = as.numeric(para_BP_individual[9,i+2])
QbrainC <- as.numeric(para_BP_individual[22,i+2])
QmuscleC <-as.numeric(para_BP_individual[24,i+2])

VplasmaC <-as.numeric(para_BP_individual[16,i+2])
VfatC <- as.numeric(para_BP_individual[15,i+2])
VliverC <- as.numeric(para_BP_individual[10,i+2])
VbrainC <- as.numeric(para_BP_individual[25,i+2])
VgonadC <- as.numeric(para_BP_individual[14,i+2])
VskinC <-  as.numeric(para_BP_individual[13,i+2])
VrichC <- as.numeric(para_BP_individual[26,i+2])
VbodygC <- VplasmaC 
VbodysC <- VplasmaC
...
vreabsorptiong <- vreabsorptiongC*bw^0.75 # 
vreabsorptions <- vreabsorptionsC*bw^0.75 # 
EHRrate <- EHRrateC/(bw^0.25) # (1/h) |EHR of BPAG
k0 <- k0C/bw^0.25
ge <- geC/bw^0.25 # (1/h) |Gastric emptying of BPA
k1 <- k1C/bw^0.25 # (1/h) 
k4 <- k4C/bw^0.25 # (1/h) 
k4_IV <- k4C_IV/bw^0.25 # (1/h) 
vmaxliver <- vmaxliverCnew*fliverg*bw^0.75 
kGIing <- kGIingC/bw^0.25 
met2g <- 1.0-met1g 
met2s <- 1.0-met1s
...
vmaxlivers <- vmaxliversCnew*flivers*bw^0.75 
kGIins <- kGIinsC/bw^0.25 
vmaxgutg <- vmaxgutgCnew*fgutg*bw^0.75
vmaxguts <- vmaxgutsC*fguts*bw^0.75 
kenterobpag<- kenterobpagC/bw^0.25
kenterobpas <- kenterobpasC/bw^0.25
oral_intakerate=0
########################################################################
...
############################Dermal pars ##############
BSA= as.numeric(para_BP_individual[21,i+2])*100 #m2-dm2
...
DOSE=as.numeric(para_BP_individual[17,i+2])/MWBPS
ABS=as.numeric(para_BP_individual[18,i+2])
DOSE_d=DOSE*ABS
...
#DSC = 9.2E-9 # diffusion (cm^2/h- dm2)
#PFO= 4.45E-5 #cm/h- dm
#u1=1E-5

#17.281053  6.389016  5.704079
#DSC = 17.28E-9 # diffusion (cm^2/h- dm2)
#PFO= 6.39E-5 #cm/h- dm
#u1=5.7E-5

DSC =theta[1]/1000/1000/1000
PFO= theta[2]/1000/100
u1=  theta[3]/1000/100
...
try_bps_d1<-ode(y=yini_bps_dermal, func=PBTKmod_bps_d1, 
                    times=zeit, parms=para_bps_dermal, method="lsoda")


total_u=try_bps_d1[,"Aurinebpa"]+try_bps_d1[,"Aurinebpag"] 
t_u=total_u/(DOSE_d)*1000*1000 #尿液中BPS＋BPSg的总量的时间序列
f_u=try_bps_d1[,"Aurinebpa"]/(DOSE_d)*1000*1000 #尿液中未结合BPS的量的时间序列
urine_out_per[,i]=t_u
urine_out_per[,i+4]=f_u
urine_init_per[,i]=t_u[t_data/0.005+1]
urine_init_per[,i+4]=f_u[t_data/0.005+1]

  }
  
  # Calculate sum of squared errors.
  SE1 =((urine_init_per[which(x202111_BPS$ID=="a"),1] - 
          c_data[which(x202111_BPS$ID=="a")])/142.853614 )** 2 
  SE2 = ((urine_init_per[which(x202111_BPS$ID=="c"),2]- 
           c_data[which(x202111_BPS$ID=="c")])/ 96.037827)** 2  
  SE3 = ((urine_init_per[which(x202111_BPS$ID=="f"),3]- 
           c_data[which(x202111_BPS$ID=="f")])/ 91.694228)** 2  
  SE4 =  ((urine_init_per[which(x202111_BPS$ID=="h"),4]- 
            c_data[which(x202111_BPS$ID=="h")])/135.018903)** 2
  
  SE5 =((urine_init_per[which(x202111_BPS$ID=="a"),5] - 
         f_data[which(x202111_BPS$ID=="a")])/ 10.240965)** 2 
  SE6 = ((urine_init_per[which(x202111_BPS$ID=="c"),6]- 
           f_data[which(x202111_BPS$ID=="c")])/ 10.590446)** 2 
  SE7 =( (urine_init_per[which(x202111_BPS$ID=="f"),7]- 
           f_data[which(x202111_BPS$ID=="f")])/ 13.855707 )** 2  
  SE8 = ( (urine_init_per[which(x202111_BPS$ID=="h"),8]- 
            f_data[which(x202111_BPS$ID=="h")])/ 10.644076 )** 2
  print(SE8)
  print("上面是SE8")
  print(SE6)
  print("上面是SE6")
  SSE = sum(SE5,SE6,SE7,SE8,SE1,SE2,SE3,SE4)
  print(SSE)
  print("上面是SE6")
  return(SSE)
}

cost_fun_bps1(theta_s1,t_data_s1,c_data_s1,f_data_s1) #1572.01 3327.244  20.26875
#cost_fun_bps1(theta_s2,t_data_s1,c_data_s1,f_data_s1) #1606.442 3286.931 19.58416

################OPTI##################

opt_res_bps4 = optim(par = theta_s1, fn=cost_fun_bps1, gr=NULL,

...
time = torch.arange(0,75,0.005)
paradata= torch.tensor([[17.28, 6.39, 5.7],[14.91,2.78,4.707],[21.333, 9.666, 7.42],[19.921, 11, 2.4212],[23.032,7.2223,4.99],
                [13.53,10.11,5.444],[18.888,9.999,8.888],[15.5,10.5,7.5],[12.45,6.84,6.75],[20.8,4.61,9.888]])



def BPS_BPTK_MultiParas_torch(t = time,volunteer_ID =j, paras = paradata ,mode = '63'):

	device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

	plasma_data = torch.zeros(paras.shape[0],15000)
	...
	para_BP_individual = torch.tensor(para_BP_individual).float()
	...
	class BPS_BPTK(nn.Module):
		def __init__(self, DSC_0, PFO_0, u1_0):
			super(BPS_BPTK, self).__init__()
			self.DSC_0 = DSC_0
			self.PFO_0 = PFO_0
			self.u1_0 = u1_0	
		def forward(self, t, XX):
			...
			DSC =self.DSC_0/1000/1000/1000   #角质层中的有效扩散系数（cm2/h- dm2）  
			PFO= self.PFO_0/1000/100 #毛囊的渗透系数（cm/h- dm）
			u1=  self.u1_0/1000/100  #由脱屑而向皮肤表面转移的速度（cm/min）
			...
			return torch.tensor([Rdose,dAFO,
	...
	for i in range(paras.shape[0]):
		...
		PFO_0 =  paras[i,1]
		u1_0 =  paras[i,2]
		func =	BPS_BPTK(DSC_0, PFO_0 , u1_0)
		result = odeint(func= func, y0 =y0, t = t  )

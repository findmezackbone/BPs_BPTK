...
Data_origin = np.load("Python\optim\DataFromBPTK\plasma28+urine15x2\plasma28+urine15x2_zzc.npy")  #输入数据
...
def test_NewData_NN(origin_para = paras , model = bestmodel, model_path = best_model_path, Data = Data_origin, sampling_time_range_plasma = time_range, sampling_time_range_urine = time_range1):
    ...
    err_urine15 = np.zeros((np.shape(origin_para)[0],2))
    err_urineg15 = np.zeros((np.shape(origin_para)[0],2))
    ...
    for i in range(np.shape(origin_para)[0]):
        ...
        err_urine15[i,0] =  np.linalg.norm(urine_fromNN[i,sampling_time_index_urine]-urine_true[i,sampling_time_index_urine], ord=1)/np.shape(sampling_time_index_urine)[0]
        err_urine15[i,1] =  np.linalg.norm(urine_fromNN[i,sampling_time_index_urine]/(urine_true[i,sampling_time_index_urine]+1E-16)-1, ord=1)/np.shape(sampling_time_index_urine)[0]
 
        err_urineg15[i,0] =  np.linalg.norm(urineg_fromNN[i,sampling_time_index_urine]-urineg_true[i,sampling_time_index_urine], ord=1)/np.shape(sampling_time_index_urine)[0]
        err_urineg15[i,1] =  np.linalg.norm(urineg_fromNN[i,sampling_time_index_urine]/(urineg_true[i,sampling_time_index_urine]+1E-16)-1, ord=1)/np.shape(sampling_time_index_urine)[0]

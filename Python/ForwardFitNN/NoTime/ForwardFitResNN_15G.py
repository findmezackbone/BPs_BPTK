...
# 准备数据
y1 = np.load("Python\optim\DataFromBPTK\\urine15\\urinebpsg15_zzc.npy")  #输入标签数据
y2 = np.load("Python\optim\DataFromBPTK\\urine15\\urinebpsg15_SG.npy")  #输入标签数据
...
class CustomResNN(nn.Module):
    def __init__(self,hyperparas):
        ...
        self.output_dim = hyperparas['output_dim'] #15
...
#超参数合集
hyperparas = {'input_dim':3,'hidden_dim':30,'hidden_nums':3,'output_dim':15,'block_layer_nums':3}
learning_rate = 0.001
num_epochs = 300
...
def criterion(output,label,mode = 1):
    ...
    if mode == 4:
        ...
        loss1 = loss_function(output,label)
        loss3 = torch.mean((output_reverse - label_reverse)**2)
        loss = 0.4*1E5*loss1 + 0.5*1E4*loss2 + 0.3*1E6*loss3
        return loss, 0.4*1E5*loss1, 0.5*1E4*loss2,0.3*1E6*loss3
...
_,_,result_True = BPS_BPTK_MultiParas (t = time,volunteer_ID =id, paras= example_paras,mode = '63')

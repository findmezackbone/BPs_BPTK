...
plt.plot(t,BPS_result_1,label = 'python22_result')
plt.xlabel('time(h)')
plt.ylabel('concentration of BPS in plasma')
plt.legend()
plt.show()

plt.plot(t,abs(result[:,26]-original_result[:,26]),label = 'Python_result')
plt.xlabel('time(h)')
plt.ylabel('Absolute Error of R and Python Results')
plt.show()

plt.plot(t,abs(result[:,26]/original_result[:,26]-1),label = 'Python_result')
plt.xlabel('time(h)')
